...
then
    ...
    SERVICE_NAME_FLOWZ_API_DOC="$SERVICE_NAME_FLOWZ_API_DOC_MASTER";
  }
...
then
    {
      ...
      SERVICE_NAME_FLOWZ_API_DOC="$SERVICE_NAME_FLOWZ_API_DOC_DEVELOP";
...
then
    {
      ...
      SERVICE_NAME_FLOWZ_API_DOC="$SERVICE_NAME_FLOWZ_API_DOC_STAGING";
    }    
else
  {
      echo "call $TRAVIS_BRANCH branch"
      ...
      SERVICE_NAME_FLOWZ_API_DOC="$SERVICE_NAME_FLOWZ_API_DOC_QA";
  }
fi

SERVICE_ID_FLOWZ_API_DOC=`curl -u ""$RANCHER_ACCESSKEY":"$RANCHER_SECRETKEY"" -X GET -H 'Accept: application/json' -H 'Content-Type: application/json' "$RANCHER_URL/v2-beta/projects/$ENV_ID/services?name=$SERVICE_NAME_FLOWZ_API_DOC" | jq '.data[].id' | tr -d '"'`
echo $SERVICE_ID_FLOWZ_API_DOC
...
echo "waiting for service to upgrade "
    while true; do
      ...
      case `curl -u ""$RANCHER_ACCESSKEY":"$RANCHER_SECRETKEY"" \
          ...
          "$RANCHER_URL/v2-beta/projects/$ENV_ID/services/$SERVICE_ID_FLOWZ_API_DOC/" | jq '.state'` in
          "\"upgraded\"" )
              ...
              curl -u ""$RANCHER_ACCESSKEY":"$RANCHER_SECRETKEY"" \
                ...
                -H 'Accept: application/json' \
                -H 'Content-Type: application/json' \
                "$RANCHER_URL/v2-beta/projects/$ENV_ID/services/$SERVICE_ID_FLOWZ_API_DOC?action=finishupgrade"
              break ;;
          "\"upgrading\"" )
